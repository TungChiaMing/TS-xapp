...
bool XappRmr::xapp_rmr_send(xapp_rmr_header *hdr, void *payload){
	...
	thread_id << my_id; //thread_id << my_id
	...
	//fprintf(stderr, "Ken Payload %s\n", payload);  
	//mdclog_write(MDCLOG_INFO, "Ken Payload %s" , payload); 
	_xapp_send_buff->len = hdr->payload_length;
	...
	while(rmr_attempts > 0){
		...
		//fprintf(stderr, "Ken buff %s\n", _xapp_send_buff); 
		//mdclog_write(MDCLOG_INFO, "Ken buff %s" , _xapp_send_buff); 
		if(!_xapp_send_buff) {
